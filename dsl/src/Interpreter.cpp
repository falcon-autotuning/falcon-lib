...
typing::FunctionResult Interpreter::run(const atc::AutotunerDecl &autotuner,
  ...
  typing::ParameterMap saved_variables = std::move(variables_);
  ...
  try {
    ...
    typing::FunctionResult result = extract_outputs(autotuner);
...
Interpreter::execute_state(const atc::StateDecl &state,
                           std::vector<typing::RuntimeValue> &input_params) {
