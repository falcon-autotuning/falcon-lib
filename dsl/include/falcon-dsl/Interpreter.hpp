...
public:
   ...
   */
  typing::FunctionResult run(const atc::AutotunerDecl &autotuner,
...
private:
  ...
  ControlFlow execute_state(const atc::StateDecl &state,
                            std::vector<typing::RuntimeValue> &input_param);
  const atc::StateDecl *find_state(const atc::AutotunerDecl &autotuner,
                                   const std::string &name);
  typing::FunctionResult extract_outputs(const atc::AutotunerDecl &autotuner);
