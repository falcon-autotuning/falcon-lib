...
std::string DefinitionProvider::word_at(const std::string &text, unsigned line,
                                        unsigned character) {
  unsigned cur_line = 0;
  size_t line_start = 0;
...
DefinitionProvider::definition(const FalconDocument &doc,
  ...
  for (const auto &sym : doc.symbols) {
    if (sym.name == word && sym.line > 0) {
      ...
      unsigned lsp_col = sym.col > 0 ? static_cast<unsigned>(sym.col - 1) : 0u;
      ::lsp::Location loc;
      loc.uri = ::lsp::FileUri::fromPath(::lsp::Uri::parse(doc.uri).path());
      loc.range = ::lsp::Range{
