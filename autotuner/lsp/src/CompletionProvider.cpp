...
static const std::vector<std::string> KEYWORDS = {
    "autotuner", "routine", "state", "start", "terminal",
    "if", "else", "true", "false", "nil",
    "int", "float", "bool", "string",
    "Quantity", "Config", "Connection", "Connections", "Gname",
    "DeviceCharacteristic", "DeviceCharacteristicQuery", "Error", "FatalError"
};

std::vector<::lsp::CompletionItem> CompletionProvider::complete(
    ...
    for (const auto& kw : KEYWORDS) {
        ...
        item.label = kw;
        item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Keyword};
    ...
    for (const auto& name : BUILTIN_NAMES) {
        ...
        if (sig) {
            ::lsp::CompletionItem item;
            item.label = name;
            item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Function};
            item.detail = "builtin";
            items.push_back(std::move(item));
        }
    }
    ...
    for (const auto& sym : doc.symbols) {
        ...
        if (sym.kind == "autotuner") {
            item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Class};
        } else if (sym.kind == "state") {
            item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Module};
        } else if (sym.kind == "routine") {
            item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Function};
        } else if (sym.kind == "input_param" || sym.kind == "output_param"
                   || sym.kind == "var") {
            item.kind = ::lsp::CompletionItemKindEnum{::lsp::CompletionItemKind::Variable};
