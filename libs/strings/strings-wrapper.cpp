...
static void pack_match(RegexMatchSP p, FalconResultSlot *out, int32_t *oc) {
    ...
    out[0].value.opaque.deleter   = [](void *q){ delete static_cast<RegexMatchSP*>(q); };
    *oc = 1;
}
...
void Contains(const FalconParamEntry *p, int32_t pc,
...
void StartsWith(const FalconParamEntry *p, int32_t pc,
                FalconResultSlot *out, int32_t *oc) {
    auto pm     = unpack_params(p, pc);
    auto s      = std::get<std::string>(pm.at("s"));
...
void EndsWith(const FalconParamEntry *p, int32_t pc,
              FalconResultSlot *out, int32_t *oc) {
    auto pm     = unpack_params(p, pc);
    auto s      = std::get<std::string>(pm.at("s"));
...
void Length(const FalconParamEntry *p, int32_t pc,
            FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
...
void IsEmpty(const FalconParamEntry *p, int32_t pc,
             FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
...
void ToUpper(const FalconParamEntry *p, int32_t pc,
             FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
...
void ToLower(const FalconParamEntry *p, int32_t pc,
             FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
...
void TrimSpace(const FalconParamEntry *p, int32_t pc,
               FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
    // Only trim spaces and tabs, not newlines
    size_t start = s.find_first_not_of(" \t\r\f\v");
    if (start == std::string::npos) { pack_results(FunctionResult{std::string("")}, out, 16, oc); return; }
...
void Replace(const FalconParamEntry *p, int32_t pc,
             FalconResultSlot *out, int32_t *oc) {
    auto pm   = unpack_params(p, pc);
    auto s    = std::get<std::string>(pm.at("s"));
...
void ReplaceAll(const FalconParamEntry *p, int32_t pc,
                FalconResultSlot *out, int32_t *oc) {
    auto pm   = unpack_params(p, pc);
    auto s    = std::get<std::string>(pm.at("s"));
    auto from = std::get<std::string>(pm.at("from"));
...
void Concat(const FalconParamEntry *p, int32_t pc,
            FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
...
void Substring(const FalconParamEntry *p, int32_t pc,
               FalconResultSlot *out, int32_t *oc) {
    auto pm  = unpack_params(p, pc);
    auto s   = std::get<std::string>(pm.at("s"));
    ...
    if (start >= (int64_t)s.size()) {
        pack_results(FunctionResult{std::string("")}, out, 16, oc); return;
    }
...
void IndexOf(const FalconParamEntry *p, int32_t pc,
             FalconResultSlot *out, int32_t *oc) {
    auto pm  = unpack_params(p, pc);
    auto s   = std::get<std::string>(pm.at("s"));
    auto sub = std::get<std::string>(pm.at("sub"));
...
void StripAnsi(const FalconParamEntry *p, int32_t pc,
               FalconResultSlot *out, int32_t *oc) {
    auto pm = unpack_params(p, pc);
    auto s  = std::get<std::string>(pm.at("s"));
...
void STRUCTRegexMatchFull(const FalconParamEntry *p, int32_t pc,
                          FalconResultSlot *out, int32_t *oc) {
    pack_results(FunctionResult{get_opaque<RegexMatch>(p, pc, "this")->full},
                 out, 16, oc);
}
...
void RegexTest(const FalconParamEntry *p, int32_t pc,
               FalconResultSlot *out, int32_t *oc) {
    auto pm      = unpack_params(p, pc);
    auto pattern = std::get<std::string>(pm.at("pattern"));
    auto text    = std::get<std::string>(pm.at("text"));
...
void RegexReplace(const FalconParamEntry *p, int32_t pc,
                  FalconResultSlot *out, int32_t *oc) {
    auto pm          = unpack_params(p, pc);
    auto s           = std::get<std::string>(pm.at("s"));
    auto pattern     = std::get<std::string>(pm.at("pattern"));
    ...
    std::string result = s;
    try {
        result = std::regex_replace(s, std::regex(unescape_pattern(pattern)), replacement);
    } catch (const std::regex_error &) {}
    pack_results(FunctionResult{result}, out, 16, oc);
}
