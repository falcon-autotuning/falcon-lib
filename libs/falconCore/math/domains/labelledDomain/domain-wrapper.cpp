...
void STRUCTLabelledDomainNewFromKnob(const FalconParamEntry *params,
  ...
  auto ld = std::make_shared<LabelledDomain>(default_name,
                                             std::make_pair(min, max),
                                             conn, inst_type,
                                             lesser, greater, unit, desc);
...
void STRUCTLabelledDomainFromPort(const FalconParamEntry *params,
  ...
  bool   greater = std::get<bool>(pm.at("greaterBoundContained"));
  auto   ld      = LabelledDomain::from_port(std::make_pair(max, min), port,
                                             lesser, greater);
...
// ContainsDomain(this: LabelledDomain, other: LabelledDomain) -> (bool contains)
void STRUCTLabelledDomainContainsDomain(const FalconParamEntry *params,
  ...
  pack_results(
      FunctionResult{
          self->contains_domain(std::static_pointer_cast<Domain>(other))},
      out, 16, oc);
}
