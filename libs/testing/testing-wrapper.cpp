...
struct TestSuite {
  std::string name;
  std::vector<TestContextSP> results;
...
using TestSuiteSP = std::shared_ptr<TestSuite>;

// ── Helpers
// ───────────────────────────────────────────────────────────────────
static void pack_opaque_ctx(TestContextSP ctx, FalconResultSlot *out,
                            int32_t *out_count) {
  out[0] = {};
  out[0].tag = FALCON_TYPE_OPAQUE;
  out[0].value.opaque.type_name = "TestContext";
  out[0].value.opaque.ptr = new TestContextSP(std::move(ctx));
  out[0].value.opaque.deleter = [](void *p) {
    delete static_cast<TestContextSP *>(p);
  };
  *out_count = 1;
}

static void pack_opaque_suite(TestSuiteSP suite, FalconResultSlot *out,
                              int32_t *out_count) {
  out[0] = {};
  out[0].tag = FALCON_TYPE_OPAQUE;
  out[0].value.opaque.type_name = "TestSuite";
  out[0].value.opaque.ptr = new TestSuiteSP(std::move(suite));
  out[0].value.opaque.deleter = [](void *p) {
    delete static_cast<TestSuiteSP *>(p);
  };
  *out_count = 1;
}
...
void STRUCTTestContextName(const FalconParamEntry *params, int32_t param_count,
                           FalconResultSlot *out, int32_t *out_count) {
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
  pack_results(FunctionResult{ctx->name}, out, 16, out_count);
}

void STRUCTTestContextPassed(const FalconParamEntry *params,
...
void STRUCTTestContextFailureCount(const FalconParamEntry *params,
                                   int32_t param_count, FalconResultSlot *out,
                                   int32_t *out_count) {
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
  pack_results(FunctionResult{(int64_t)ctx->failure_count()}, out, 16,
               out_count);
}

void STRUCTTestContextExpectTrue(const FalconParamEntry *params,
                                 int32_t param_count, FalconResultSlot *out,
                                 int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectFalse(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectIntEq(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectIntNe(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectIntLt(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectIntGt(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectFloatEq(const FalconParamEntry *params,
                                    int32_t param_count, FalconResultSlot *out,
                                    int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectStrEq(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestContextExpectStrNe(const FalconParamEntry *params,
                                  int32_t param_count, FalconResultSlot *out,
                                  int32_t *out_count) {
  auto pm = unpack_params(params, param_count);
  auto ctx = get_opaque<TestContext>(params, param_count, "this");
...
void STRUCTTestSuiteNew(const FalconParamEntry *params, int32_t param_count,
  ...
  auto suite = std::make_shared<TestSuite>();
  suite->name = name;
  pack_opaque_suite(std::move(suite), out, out_count);
}

void STRUCTTestSuiteAddResult(const FalconParamEntry *params,
                              int32_t param_count, FalconResultSlot *out,
                              int32_t *out_count) {
  auto suite = get_opaque<TestSuite>(params, param_count, "this");
  auto ctx = get_opaque<TestContext>(params, param_count, "ctx");
  suite->results.push_back(ctx);
  pack_nil_error(out, out_count);
}

void STRUCTTestSuiteTotalCount(const FalconParamEntry *params,
                               int32_t param_count, FalconResultSlot *out,
                               int32_t *out_count) {
  auto suite = get_opaque<TestSuite>(params, param_count, "this");
...
void STRUCTTestSuitePassedCount(const FalconParamEntry *params,
                                int32_t param_count, FalconResultSlot *out,
                                int32_t *out_count) {
  auto suite = get_opaque<TestSuite>(params, param_count, "this");
...
void STRUCTTestSuiteFailedCount(const FalconParamEntry *params,
                                int32_t param_count, FalconResultSlot *out,
                                int32_t *out_count) {
  auto suite = get_opaque<TestSuite>(params, param_count, "this");
...
void STRUCTTestSuitePrintSummary(const FalconParamEntry *params,
                                 int32_t param_count, FalconResultSlot *out,
                                 int32_t *out_count) {
  auto suite = get_opaque<TestSuite>(params, param_count, "this");
  suite->print_summary();
