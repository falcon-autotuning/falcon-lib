...
// Device
//
// C++ wrapper around the Python Device class defined in quantum_dot_device.py.
...
public:
    ...
    /// Keys must match the expected YAML keys of quantum_dot_device.py.
    ...
    int                      n_dots() const;
    std::vector<std::string> gate_names() const;
    std::map<std::string, double> voltages() const;

    // ── Scans ─────────────────────────────────────────────────────────────────

    ScanResult2D scan_2d(
        const std::string& x_gate,
    ...
    ScanResult1D scan_1d(
        const std::string& gate,
        std::pair<double, double> v_range,
        int resolution
    );

    ScanResultRay scan_ray(
        const std::map<std::string, double>& start,
