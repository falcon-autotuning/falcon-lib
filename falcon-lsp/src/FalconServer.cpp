...
FalconServer::FalconServer(::lsp::MessageHandler& handler)
    : handler_(handler) {
    setup_handlers();
}
...
void FalconServer::setup_handlers() {
    handler_.add<::lsp::requests::Initialize>(
        [](::lsp::requests::Initialize::Params&&) {
            ...
            sync_opts.change = ::lsp::TextDocumentSyncKindEnum{
    ...
    handler_.add<::lsp::notifications::TextDocument_DidOpen>(
        [this](::lsp::notifications::TextDocument_DidOpen::Params&& params) {
            ...
            auto& doc = analyze(std::string{params.textDocument.uri.path()},
                                text);
    ...
    handler_.add<::lsp::notifications::TextDocument_DidChange>(
        [this](::lsp::notifications::TextDocument_DidChange::Params&& params) {
            ...
            if (std::holds_alternative<
                ...
                text = std::get<
                    ::lsp::TextDocumentContentChangeEvent_Text>(change).text;
            } else {
                text = std::get<
                    ::lsp::TextDocumentContentChangeEvent_Range_Text>(change)
                           .text;
            }
            const std::string uri =
                std::string{params.textDocument.uri.path()};
