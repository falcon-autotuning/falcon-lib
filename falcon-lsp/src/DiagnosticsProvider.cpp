...
std::vector<::lsp::Diagnostic> DiagnosticsProvider::diagnostics(
    ...
    for (const auto& err : doc.parse_errors) {
        ...
        unsigned start_line = err.first_line > 0
                                  ? static_cast<unsigned>(err.first_line - 1)
                                  : 0u;
        unsigned start_col = err.first_column > 0
                                 ? static_cast<unsigned>(err.first_column - 1)
                                 : 0u;
