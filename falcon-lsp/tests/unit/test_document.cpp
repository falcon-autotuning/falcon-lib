...
autotuner Calc (int a) -> (int result) {
  ...
  state run {
    ...
    for (const auto& sym : doc.symbols) {
        if (sym.name == "Calc" && sym.kind == "autotuner") found_autotuner = true;
        if (sym.name == "a" && sym.kind == "input_param") found_param = true;
    }
...
TEST(FalconDocument, ParseRoutine) {
    FalconDocumentParser parser;
    // routines must follow autotuner declarations; empty output is `-> ` (no parens)
    const std::string src = R"(
